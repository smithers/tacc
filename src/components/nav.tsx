        ...
        <div className="flex items-center gap-6">
          {mainLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`relative text-sm transition-colors pb-1 after:absolute after:bottom-0 after:left-0 after:h-[2px] after:bg-current after:transition-all after:duration-200 ${
                pathname === link.href
                  ? "font-medium text-black dark:text-zinc-50 after:w-full"
                  : "text-black/60 hover:text-black hover:font-medium dark:text-zinc-400 dark:hover:text-zinc-200 after:w-0 hover:after:w-full"
              }`}
            >
              {link.label}
            </Link>
        ...
        {!logoOnly && name && (
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="flex items-center gap-1 py-2 text-sm text-black/60 transition-colors hover:text-black hover:font-medium dark:text-zinc-400 dark:hover:text-zinc-200"
            >
              Hi, {name}
              <svg
